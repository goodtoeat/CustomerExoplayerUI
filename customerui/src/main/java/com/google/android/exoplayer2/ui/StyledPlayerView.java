...
public class StyledPlayerView extends FrameLayout implements AdViewProvider {
  ...
  @Nullable private final Button lastSeen;
  ...
  public StyledPlayerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    lastSeen = findViewById(R.id.exo_last_seen);
    ...
    if (isDpadKey && useController() && !controller.isFullyVisible()) {
      ...
      if (dpadKeyListener!=null) {
        if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_CENTER) {
          if (lastSeen.getVisibility() == VISIBLE){
            lastSeen.callOnClick();
            bypass = true;
          }else {
            bypass = dpadKeyListener.dpadCenter();
          }
        } else if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_UP) {
          bypass = dpadKeyListener.dpadUp();
        } else if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_DOWN) {
          bypass = dpadKeyListener.dpadDown();
        } else if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_LEFT) {
          bypass = dpadKeyListener.dpadLeft();
        } else if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_RIGHT) {
          bypass = dpadKeyListener.dpadRight();
        } else if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_UP_LEFT) {
          bypass = dpadKeyListener.dpadUpLeft();
        } else if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_UP_RIGHT) {
          bypass = dpadKeyListener.dpadUpRight();
        } else if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_DOWN_LEFT) {
          bypass = dpadKeyListener.dpadDownLeft();
        } else if (event.getKeyCode() == KeyEvent.KEYCODE_DPAD_DOWN_RIGHT) {
          bypass = dpadKeyListener.dpadDownRight();
        }
      }
  ...
  public interface LastSeenClickListener {
    void skipTo();
  }
  public void setLastSeen(String buttonText, LastSeenClickListener listener){
    lastSeen.setText(buttonText);
    lastSeen.setOnClickListener(new OnClickListener() {
      @Override
  ...
  public void setLastWatchShowing(boolean showing){
    if (showing){
      lastSeen.setVisibility(VISIBLE);
      postDelayed(() -> lastSeen.requestFocus(),500);
      postDelayed(() -> lastSeen.setVisibility(GONE),10000);
    } else {
  ...
  /**
   * 返回值 true: 不做後續流程, 結束事件
   * 返回值 false: 繼續走系統流程
   */
  public interface CustomerDpadKeyListener{
    boolean dpadUp();
    boolean dpadUpRight();
    boolean dpadRight();
    boolean dpadDownRight();
    boolean dpadDown();
    boolean dpadDownLeft();
    boolean dpadLeft();
    boolean dpadUpLeft();
    boolean dpadCenter();
  }

  private CustomerDpadKeyListener dpadKeyListener;

  public void setDpadKeyListener(CustomerDpadKeyListener listener){
    dpadKeyListener = listener;
  }

  public void showRewindAndFastForwardButton(){
  ...
  public void setTimeSkipPerDpadClick(int timeMs){
    controller.setTimeSkipPerDpadClick(timeMs);
  }
